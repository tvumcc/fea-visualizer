...
Mesh::Mesh(std::string path) {
    ...
    for (int s = 0; s < shapes.size(); s++) {
        ...
        for (int f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
            ...
            for (int v = 0; v < fv; v++) {
                auto [vertex_index, normal_index, texcoord_index] = shapes[s].mesh.indices[index_offset + v];
                tinyobj::real_t vx = attrib.vertices[3 * static_cast<size_t>(vertex_index) + 0];
                tinyobj::real_t vy = attrib.vertices[3 * static_cast<size_t>(vertex_index) + 1];
                tinyobj::real_t vz = attrib.vertices[3 * static_cast<size_t>(vertex_index) + 2];
                ...
                if (normal_index >= 0) {
                    nx = attrib.normals[3 * static_cast<size_t>(normal_index) + 0];
                    ny = attrib.normals[3 * static_cast<size_t>(normal_index) + 1];
                    nz = attrib.normals[3 * static_cast<size_t>(normal_index) + 2];
                }

                if (texcoord_index >= 0) {
                    tx = attrib.texcoords[2 * static_cast<size_t>(texcoord_index) + 0];
                    ty = attrib.texcoords[2 * static_cast<size_t>(texcoord_index) + 1];
                }
